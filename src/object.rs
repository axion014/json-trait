...
use cc_traits::{Clear, Collection, Get, GetMut, Len, MapInsert, Remove};

use crate::{ForeignJson, ForeignMutableJson};

pub trait Object<T: ForeignJson>:
	Collection<Item = T> + for<'a> Get<&'a str> + for<'a> Index<&'a str, Output = T> + Len + PartialEq + FromIterator<(String, T)> + std::fmt::Debug
{
...
pub trait MutableObject<T: ForeignMutableJson>:
	Object<T> + for<'a> GetMut<&'a str> + MapInsert<String> + for<'a> Remove<&'a str> + Clear + IntoIterator<Item = (String, T)>
{
